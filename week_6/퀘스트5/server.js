...
async function initDb() {
  ...
  await admin.ensureAdminSchema(pool); // shop_admin_todos (사장님 대시보드 할 일)
  await seedProducts();
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    // ---- 사장님 대시보드 (admin 계정만) ----
    // 데이터는 카페 운영 DB(cafe_*)에서 집계하고, 브리핑은 AI 가 생성한다.
    if (pathname.startsWith('/api/admin/')) {
      ...
      try {
        ...
        if (pathname === '/api/admin/dashboard') {
          if (method !== 'GET') return sendJSON(res, 405, { success: false, message: '허용되지 않은 메서드입니다.' });
          return sendJSON(res, 200, await admin.getDashboard(pool));
        }

        if (pathname === '/api/admin/briefing') {
          if (method !== 'POST') return sendJSON(res, 405, { success: false, message: '허용되지 않은 메서드입니다.' });
          return sendJSON(res, 200, await admin.generateBriefing(pool));
        }

        if (pathname === '/api/admin/todos') {
          if (method === 'GET') return sendJSON(res, 200, await admin.listTodos(pool));
          if (method === 'POST' || method === 'PUT' || method === 'DELETE') {
            const body = await readBody(req);
            if (method === 'POST') return sendJSON(res, 201, await admin.addTodo(pool, body));
            if (method === 'PUT') return sendJSON(res, 200, await admin.toggleTodo(pool, body));
            return sendJSON(res, 200, await admin.removeTodo(pool, body));
          }
          return sendJSON(res, 405, { success: false, message: '허용되지 않은 메서드입니다.' });
        }

        return sendJSON(res, 404, { success: false, message: '알 수 없는 관리자 API 입니다.' });
      } catch (err) {
        const status = err.status || 500;
        if (status >= 500) console.error('관리자 API 오류:', err);
        return sendJSON(res, status, { success: false, message: err.message || '서버 오류' });
      }
    }

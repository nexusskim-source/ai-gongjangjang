...
// ⚠️ 이 Supabase 는 여러 앱이 공유한다. 이 디저트 쇼핑몰 앱은 전용 테이블
//    'shop_users' 만 사용한다. (기존 users/community_users 등은 건드리지 않는다.)
...
// ---------- DB 스키마 (shop_users 테이블) ----------
// 이 앱 전용 회원 테이블만 생성한다. (다른 앱 테이블은 건드리지 않는다.)
...
async function signup(pool, body) {
  ...
  const exists = await pool.query('SELECT 1 FROM shop_users WHERE username = $1', [username]);
  if (exists.rows.length > 0) throw httpError(409, '이미 사용 중인 아이디입니다.');

  let userId;
  try {
    const { rows } = await pool.query(
      'INSERT INTO shop_users (username, password_hash) VALUES ($1,$2) RETURNING id',
...
async function login(pool, body) {
  ...
  const { rows } = await pool.query('SELECT * FROM shop_users WHERE username = $1', [username]);
